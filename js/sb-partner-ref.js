 ...
 *
 * Два хранилища. localStorage помнит метку 30 дней и ставится из ?ref= или
 * ?utm_campaign= — этого хватает обычному партнёру. Плюс кука sb_ref: её
 * ставит Vercel заголовком на коротких адресах вида /r/<партнёр> и живёт она
 * 400 дней, потолок Chrome. Кука важнее не сроком, а тем, что Safari по
 * правилам ITP режет до семи суток всё, что поставлено джаваскриптом, —
 * серверную не режет. Читаем оба: сначала адрес, потом куку, потом хранилище.
...
(function () {
  ...
  /* Кука sb_ref: живёт 400 дней и переживает Safari, потому что поставлена
     сервером, а не этим скриптом. Своей куку здесь не ставим — из JS Safari
     всё равно урезал бы её до недели. */
  function readFromCookie() {
    try {
      var m = String(document.cookie || "").match(/(?:^|;\s*)sb_ref=([^;]*)/);
      return m ? sanitizeRef(decodeURIComponent(m[1])) : null;
    } catch (e) { return null; }
  }
  ...
  var ref = readFromUrl();
  if (ref) save(ref); else ref = readFromCookie() || load();
  var page = pageTag();
