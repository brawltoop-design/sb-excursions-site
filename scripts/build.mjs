...
const STEPS = [
  ...
  ["fix-orphan-details.mjs", "осиротевшие </details></div> от старой подмены переключателя"],
  ...
  ["add-swipe-guard.mjs", "вертикальный свайп по карусели не листает её"],
  ["add-wa-track.mjs", "счётчик кликов по WhatsApp на всех страницах, а не только на турах"],
  ...
  ["localize-jsonld-leftovers.mjs", "разметка schema.org: добивание строк, которые не поймал перевод"],
  ["localize-whatsapp-text.mjs", "язык заготовки сообщения в WhatsApp на локализованных страницах"],
  ["fix-article-counters.mjs", "счётчики статей в кнопках хаба: перевод шаблона, а не числа"],
  ["fix-proper-names-in-html.mjs", "имена собственные, которые перевод сделал обычными словами"],
