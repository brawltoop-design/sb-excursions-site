  ...
  "@graph": [
    {
      ...
      description: "A flat $3–$10 referral fee for every guest who books a Bali tour with SB Excursions.",
      inLanguage: "en",
    },
    {
      "@type": "FAQPage",
      mainEntity: [
        ["How and when do I get paid?", "Right after your guest's tour day — a flat $3–$10 per guest depending on the tour, sent however suits you: cash on Bali or a transfer. No minimum payout, no waiting for the end of the month."],
        ["What counts as a referred guest?", "Anyone who books a tour on WhatsApp and mentions your partner name. That's the whole tracking system — no links to install, no dashboards to check."],
        ...
        ["Is there a cap on referrals?", "No cap. Ten guests in a week means ten fees in a week. Groups count per guest, so a villa of six booking one tour is six fees."],
...
const styleBlock = `<style id="sb-wwu-css">
.sb-wwu{
  --sand:#f3efe7;--ink:#141412;--muted:#75716a;--line:rgba(20,20,18,.12);
  --coral:#e8552f;--dark:#161512;--sand-on-dark:#efe9dc;
  font-family:'Cina GEO','TildaSans',Arial,sans-serif;color:var(--ink);line-height:1.55;
  background:var(--sand);overflow:clip;
}
/* fonts-cinageo.css общесайтово ставит html body{background:#fff!important}
   и html h1-h4{font-weight:400!important} — возвращаем фон страницы
   и дисплейные веса (этот блок в каскаде позже и побеждает) */
html body{background-color:#f3efe7!important}
...
.sb-wwu .kicker{font-size:13px;letter-spacing:.22em;text-transform:uppercase;color:var(--muted);font-weight:600;margin:0 0 22px}
.sb-wwu .hero h1{font-size:clamp(54px,10.5vw,148px);line-height:.94;letter-spacing:-.045em;font-weight:300;margin:0;text-wrap:balance;font-family:inherit}
.sb-wwu .hero h1 em{font-style:normal;font-weight:600;color:var(--coral)}
...
.sb-wwu .btn-dark:hover{background:var(--coral);color:#fff;transform:translateY(-2px);box-shadow:0 14px 30px -14px rgba(232,85,47,.7)}
...
.sb-wwu .badge .mid{position:absolute;inset:0;display:grid;place-items:center;font-weight:700;font-size:26px;letter-spacing:-.03em;color:var(--coral)}
...
.sb-wwu .marquee span{font-size:clamp(20px,3vw,34px);font-weight:300;letter-spacing:-.02em;white-space:nowrap;padding-right:18px}
.sb-wwu .marquee b{color:var(--coral);font-weight:400;padding-right:18px}
@keyframes sbwwu-slide{to{transform:translateX(-50%)}}

/* ---------- секции ---------- */
.sb-wwu .eyebrow{font-size:13px;letter-spacing:.22em;text-transform:uppercase;color:var(--muted);font-weight:600;margin:0 0 18px}
.sb-wwu h2{font-size:clamp(36px,5.5vw,72px);font-weight:300;letter-spacing:-.04em;line-height:1.02;margin:0 0 26px;text-wrap:balance;font-family:inherit}
.sb-wwu h2 em{font-style:normal;color:var(--coral);font-weight:500}

/* сделка — тёмная секция */
.sb-wwu .deal{background:var(--dark);color:var(--sand-on-dark)}
.sb-wwu .deal h2{color:#fff}
.sb-wwu .deal .grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:1px;background:rgba(239,233,220,.14);border:1px solid rgba(239,233,220,.14);border-radius:20px;overflow:hidden;margin-top:48px}
...
.sb-wwu .deal .num b{color:var(--coral);font-weight:500}
.sb-wwu .deal .cell p{margin:14px 0 0;font-size:15px;color:rgba(239,233,220,.72)}
.sb-wwu .tiers{margin-top:46px;border:1px solid rgba(239,233,220,.16);border-radius:20px;overflow:hidden}
.sb-wwu .tier{display:grid;grid-template-columns:1fr auto;gap:16px;align-items:baseline;padding:22px 28px;border-top:1px solid rgba(239,233,220,.12)}
...
.sb-wwu .tier .t-name small{display:block;font-size:14px;color:rgba(239,233,220,.6);margin-top:4px;font-weight:400}
.sb-wwu .tier .t-fee{font-size:clamp(26px,3.4vw,44px);font-weight:500;letter-spacing:-.03em;color:var(--coral);white-space:nowrap}
.sb-wwu .deal .fine{margin:18px 4px 0;font-size:14px;color:rgba(239,233,220,.55)}
...
.sb-wwu .card .idx{font-size:12px;font-weight:700;letter-spacing:.18em;color:var(--coral);display:block;margin-bottom:8px}
...
.sb-wwu .steps li::before{content:"0" counter(st);font-size:clamp(40px,6vw,84px);font-weight:200;letter-spacing:-.05em;line-height:.9;color:var(--coral)}
...
.sb-wwu .why li{background:var(--sand);border:1px solid var(--line);border-radius:16px;padding:20px 22px;font-size:15.5px}
...
.sb-wwu .faq summary::after{content:"+";font-size:30px;font-weight:200;color:var(--coral);transition:rotate .3s ease;flex:none}
...
<div class="hero">
  ...
  <div class="badge" aria-hidden="true">
    <svg viewBox="0 0 100 100">
      ...
      <text style="font-size:9.2px;font-weight:600;letter-spacing:.18em;fill:#141412"><textPath href="#sbwwu-circ">FLAT FEE PER GUEST · PAID AFTER EVERY TOUR ·</textPath></text>
    </svg>
  ...
  <div class="wrap">
    ...
    <p class="sub rv">You know people who want to see Bali — we run the tours. Recommend us, and we pay you a <strong>flat $3–$10 for every guest who books</strong>. Paid after every tour. No contracts, no paperwork, no minimums — just your name on the booking.</p>
...
<section class="deal">
  <div class="wrap">
    ...
    <h2 class="rv">One guest = one fee.<br>Every time, <em>no exceptions.</em></h2>
    <div class="grid rv">
      <div class="cell"><div class="num"><b>$3–10</b></div><p>flat fee for every guest who books — not a percentage you have to double-check</p></div>
      ...
      <div class="cell"><div class="num">∞</div><p>no cap and no expiry: ten guests a week means ten fees a week</p></div>
    </div>
    <div class="tiers rv">
    ...
    <p class="fine rv">Per guest, not per booking — a villa of six on one tour is six fees. Current tour prices are always live at <a href="/bali/en/tour-prices">sbexcursion.com/bali/en/tour-prices</a>.</p>
  </div>
</section>
...
<section class="aud">
  <div class="wrap">
    ...
    <div class="cards">
      ...
      <div class="card rv"><img src="/images/places/diamond-beach-cliffs.jpg" alt="Diamond Beach cliffs on Nusa Penida" loading="lazy"><div class="pad"><span class="idx">04</span><h3>Bloggers &amp; creators</h3><p>Answering "who did you book with?" in your DMs every day? Give your partner name — every booked follower is a fee.</p></div></div>
    </div>
  </div>
</section>
...
<section class="steps" id="how">
  <div class="wrap">
    ...
    <ol>
      ...
      <li class="rv"><div><h3>You get paid after the tour</h3><p>Once the guest's tour day is done, your flat fee is on its way — cash on Bali or a transfer, whatever suits you. Every guest, every time.</p></div></li>
    </ol>
    <div class="gets rv">
...
<section class="faq">
  <div class="wrap">
    ...
    <details class="rv"><summary>How and when do I get paid?</summary><p class="a">Right after your guest's tour day — a flat $3–$10 per guest depending on the tour tier above, sent however suits you: cash on Bali or a transfer. No minimum payout, no waiting for the end of the month.</p></details>
    <details class="rv"><summary>What counts as a referred guest?</summary><p class="a">Anyone who books a tour on WhatsApp and mentions your partner name. That's the whole tracking system — no links to install, no dashboards to check, nothing your guest can do wrong.</p></details>
    ...
    <details class="rv"><summary>Is there a cap on referrals?</summary><p class="a">No cap and no expiry. Ten guests in a week means ten fees in a week — and groups count per guest, so a villa of six booking one tour is six fees.</p></details>
...
const html = await buildChromedPage(ROOT, {
  title: "Work With Us — SB Excursions Partner Program | $3–$10 Per Guest",
  description: "Send a guest, get paid: a flat $3–$10 for every guest who books a Bali tour with SB Excursions. No contracts, no paperwork — for villas, surf schools, drivers and creators.",
