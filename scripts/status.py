...
# ── РЫНКИ ────────────────────────────────────────────────────────────────
# Зачем отдельно от языков. Язык страницы — это то, что мы написали.
# Страна — то, откуда пришёл человек. Они не совпадают: английские страницы
# читают из Индонезии и Австралии, а не из США, и ведут себя эти рынки
# по-разному. Пока смотрели «по языкам», рынок был не виден.
#
# Ограничение Google, о котором надо помнить при чтении: разрез только по
# стране отдаётся полностью, но как только к стране добавляется страница
# или запрос, остаётся примерно пятая часть данных. Поэтому здесь есть
# «сколько показов у страны» и нет «какая статья работает в Испании».
h("РЫНКИ: откуда показы и откуда клики")
print("   CTR тут важнее показов: рынок с большими показами и нулевым CTR\n"
      "   означает, что нас показывают не тем людям\n")
COUNTRY = {
    "idn": "Индонезия", "usa": "США", "fra": "Франция", "esp": "Испания",
    "rus": "Россия", "aus": "Австралия", "gbr": "Британия", "deu": "Германия",
    "ita": "Италия", "bel": "Бельгия", "nld": "Нидерланды", "sgp": "Сингапур",
    "mys": "Малайзия", "chn": "Китай", "hkg": "Гонконг", "twn": "Тайвань",
    "can": "Канада", "che": "Швейцария", "aut": "Австрия", "pol": "Польша",
    "ukr": "Украина", "kaz": "Казахстан", "bra": "Бразилия", "mex": "Мексика",
    "arg": "Аргентина", "col": "Колумбия", "chl": "Чили", "prt": "Португалия",
    "ind": "Индия", "jpn": "Япония", "kor": "Корея", "tha": "Таиланд",
    "nzl": "Новая Зеландия", "irl": "Ирландия", "swe": "Швеция", "nor": "Норвегия",
}
countries = sa(["country"], 3, 31, 300)
c_tot = sum(r["impressions"] for r in countries) or 1
print(f"   {'страна':16}{'показы':>8}{'доля':>7}{'клики':>7}{'CTR':>8}")
for r in sorted(countries, key=lambda x: -x["clicks"])[:12]:
    k = r["keys"][0]; im = r["impressions"]; cl = r["clicks"]
    print(f"   {COUNTRY.get(k, k):16}{im:>8}{im/c_tot*100:>6.1f}%{cl:>7}{cl/im*100 if im else 0:>7.2f}%")
ENG = {"usa", "gbr", "can", "aus", "nzl", "irl"}
e_i = sum(r["impressions"] for r in countries if r["keys"][0] in ENG)
e_c = sum(r["clicks"] for r in countries if r["keys"][0] in ENG)
o_i, o_c = c_tot - e_i, sum(r["clicks"] for r in countries) - e_c
print(f"\n   англоязычные рынки  {e_i:>7} показов ({e_i/c_tot*100:>4.0f}%)  {e_c:>4} кликов  CTR {e_c/e_i*100 if e_i else 0:.2f}%")
print(f"   все остальные       {o_i:>7} показов ({o_i/c_tot*100:>4.0f}%)  {o_c:>4} кликов  CTR {o_c/o_i*100 if o_i else 0:.2f}%")
print("\n   ⚠ проверяя выдачу глазами, открывай google.fr / google.es / google.ru,")
print("     а не google.com — американская выдача по этим темам устроена иначе")
print("     и вывод по ней уже один раз оказался неверным")
